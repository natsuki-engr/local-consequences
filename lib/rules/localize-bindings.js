...
module.exports = {
    ...
    schema: [
      {
        ...
        properties: {
          ...
          passThroughTags: { type: 'array', items: { type: 'string' } },
        },
  ...
  create(context) {
    ...
    const passThroughTags = new Set(
      (options.passThroughTags || []).map((t) => t.toLowerCase())
    )

    const filename =
      context.filename ||
      (typeof context.getFilename === 'function' ? context.getFilename() : '')
    if (filename && ignoreFilePatterns.some((r) => r.test(filename))) {
      return {}
    }
    ...
    /** Set<VElement> populated at template:exit before finalReport runs. */
    const rootEqSet = new Set()

    function buildRootEqSet(rootTemplate) {
      // The root <template> itself is root-equivalent.
      rootEqSet.add(rootTemplate)
      // Walk down the substantive-child chain. Pass-through tags are
      // transparent: they don't count as siblings when deciding whether the
      // chain continues, and we descend into them as well.
      let cur = rootTemplate
      while (true) {
        const elemChildren = (cur.children || []).filter(
          (c) => c.type === 'VElement'
        )
        // Transparent passThrough siblings are also root-eq.
        for (const c of elemChildren) {
          const tag = (c.rawName || c.name || '').toLowerCase()
          if (passThroughTags.has(tag)) rootEqSet.add(c)
        }
        const primary = elemChildren.filter((c) => {
          const tag = (c.rawName || c.name || '').toLowerCase()
          return !passThroughTags.has(tag)
        })
        let next = null
        if (primary.length === 1) {
          next = primary[0]
        } else if (primary.length === 0 && elemChildren.length === 1) {
          // Lone child happens to be passThrough — descend into it anyway.
          next = elemChildren[0]
        }
        if (!next) break
        cur = next
    ...
    function isRootEquivalent(node) {
      ...
      if (node.type !== 'VElement') return true
      if (rootEqSet.has(node)) return true
      const tag = (node.rawName || node.name || '').toLowerCase()
      if (passThroughTags.has(tag)) return true
      return false
    }
