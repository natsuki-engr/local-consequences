...
module.exports = {
  ...
  create(context) {
    ...
    function buildRootEqSet(rootTemplate) {
      ...
      // Walk down the single-VElement-child chain. Each link is a
      // pass-through wrapper whose subtree only contains "the rest of the
      // component", so the deepest reachable element is the natural boundary.
      let cur = rootTemplate
      while (true) {
        const elemChildren = (cur.children || []).filter(
          (c) => c.type === 'VElement'
        )
        if (elemChildren.length !== 1) break
        cur = elemChildren[0]
        rootEqSet.add(cur)
      }
    }
